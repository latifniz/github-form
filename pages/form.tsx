...
export default function SubmitPage() {
  ...
  useEffect(() => {
    ...
    if (!storedUsername || !storedPassword || storedUsername !== 'test' || storedPassword !== 'gotu123') {
  ...
  return (
    <div className={styles.container}>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        {['githubUsername', 'githubPassword', 'githubEmail', 'githubToken'].map((field) => (
          <div key={field} className={styles.inputGroup}>
            <label htmlFor={field}>{field.replace('github', '').replace(/([A-Z])/g, ' $1')}</label>
            <input
              type="text"
              id={field}
              name={field}
